...
export function HomePageMetrics() {
    ...
    const [selectedApiForHits, setSelectedApiForHits] = useState('');
    const [selectedApiForLatency, setSelectedApiForLatency] = useState('');

    const hitsOptions = useMemo(() => {
        return {
            metric: 'hits',
            timeAggregation: 'day',
            apiIds: selectedApiForHits ? [selectedApiForHits.id] : [],
    ...
    const latenciesOptions = useMemo(() => {
        return {
            metric: 'latency',
            timeAggregation: 'day',
            apiIds: selectedApiForLatency ? [selectedApiForLatency.id] : [],
            ...getDatesFromTimeRange(timeRange),
        };
        ...
        <Card component="section">
            ...
            <CardContent className={classes.chartContainer}>
                <div>
                    ...
                    <ApiFilter
                        onChange={setSelectedApiForHits}
                        value={selectedApiForHits}
                    />
                    {hits ? (
                ...
                <div>
                    ...
                    <ApiFilter
                        onChange={setSelectedApiForLatency}
                        value={selectedApiForLatency}
                    />
                    {latencies ? (
                        <ResponsiveContainer width="100%" height={400}>
                            <AreaChart
 ...
 * @param {("minute"|"hour"|"day"|"week"|"month"|"hod"|"dow")} options.timeAggregation The time buckets.
 * @param {("hour"|"day"|"week"|"month"|"year"|"custom")} options.timerange The request time range.
 * @param {Array} options.apiIds Optional. Array of API identifiers
 */
